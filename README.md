...
The API covers authentication, tickets, comments, lifecycle, SLA refresh, manual escalation, role dashboard summaries, a read-only category list, and a manager-only active staff list. Students, staff, and managers each have a signed-in workspace. Students create and follow their own tickets. Staff work assigned tickets. Managers see every ticket, assign staff, and run lifecycle actions. The JWT is stored in `localStorage` under `edusupport_token` for this prototype. A production deployment could move that token to a hardened cookie. Dashboard charts, notifications, automatic reassignment, real-time updates, and automatic SLA monitoring are not implemented. See `docs/API.md`.
