...
The API covers authentication, tickets, comments, lifecycle, SLA refresh, manual escalation, role dashboard summaries, and a read-only category list. Students can sign in, open a support home, search and filter their tickets, create a ticket, read ticket detail, add a public comment, and close or reopen when the workflow allows it. Staff can open an assigned-ticket desk, update status and priority, reply publicly or add an internal note, resolve, refresh SLA, and escalate. The manager screen is still a placeholder. The JWT is stored in `localStorage` under `edusupport_token` for this prototype. A production deployment could move that token to a hardened cookie. Dashboard charts, notifications, automatic reassignment, and automatic SLA monitoring are not implemented. See `docs/API.md`.
